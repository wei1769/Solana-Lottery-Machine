...
pub fn init_lottery(
    ...
    while Pubkey::create_program_address(&[&lottery_key.pubkey().to_bytes()], &self::lottery_program_id()).is_err() {

        lottery_key = Keypair::new();
    }
    let lottery_pda = Pubkey::create_program_address(&[&lottery_key.pubkey().to_bytes()], &self::lottery_program_id()).unwrap();
...
pub fn withdraw(lottery_id:&Pubkey,authority: &Pubkey,connection: &RpcClient) ->Vec<Instruction> {
    ...
    let lottery_pda = Pubkey::create_program_address(&[&lottery_id.to_bytes()], &self::lottery_program_id()).unwrap();
...
pub fn get_ended_lotterys(pool_authority:&Pubkey,connection: &RpcClient)->Vec<Pubkey>{
    ...
    let filter = Some(vec![ 
        RpcFilterType::Memcmp(
            Memcmp{
                offset: 0,bytes:memcmp,encoding:None}),RpcFilterType::DataSize(161),
        ]);
...
pub fn find_closable_tickets(buyer:&Pubkey,connection: &RpcClient) -> Vec<(Pubkey,Pubkey)> {
    ...
    let filter = Some(vec![ 
            RpcFilterType::Memcmp(Memcmp{offset:0, bytes:account_type_cmp,encoding:None }),
            RpcFilterType::Memcmp(
                Memcmp{offset: 33,bytes:memcmp,encoding:None}),
